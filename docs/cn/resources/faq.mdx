...
<AccordionGroup>
  <Accordion title="如何在 Linux ARM64 设备上安装 Python？" id="如何在-linux-arm64-设备上安装-python">
    QDC 设备（高通嵌入式 Linux / Yocto）已预装 Python。如果你的设备未预装 Python，可使用以下方法之一：

    **方法 1 —— [Miniconda](https://docs.anaconda.com/miniconda/)（推荐）**
  ...
  <Accordion title="Windows ARM64 是否支持 Miniconda？">
    不支持。目前 Miniconda 没有原生的 Windows ARM64 安装程序。在 Windows ARM64 上，请直接从 python.org 下载官方 [Python ARM64 安装程序](https://www.python.org/ftp/python/3.13.3/python-3.13.3-arm64.exe)——详见 [Python 安装](/cn/run/python/install)。
  </Accordion>
</AccordionGroup>
