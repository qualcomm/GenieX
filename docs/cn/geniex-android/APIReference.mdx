...
data class EmbedderCreateInput(
    ...
    val config: ModelConfig,                   // 模型配置
    val plugin_id: String? = null,             // "npu" 用于 NPU，null 用于 CPU
...
data class EmbeddingConfig(
    val normalize: Boolean = true              // 归一化嵌入向量（默认：true）
)
```
...
data class AsrCreateInput(
    ...
    val config: ModelConfig,                   // 模型配置
    val plugin_id: String? = null,             // "whisper_cpp" 用于 whisper.cpp，"npu" 用于 NPU
    val device_id: String? = null              // 设备 ID（可选）
)
```
...
data class TtsCreateInput(
    val model_name: String? = null,            // 模型名称（CPU/GPU 留空）
    val model_path: String,                    // TTS 模型路径
    val vocoder_path: String? = null,          // 可选声码器路径
    val config: ModelConfig,                   // 模型配置
    val plugin_id: String? = null,             // "tts_cpp" 用于 TTS 后端
    val device_id: String? = null              // 设备 ID（可选）
)
```
...
data class TtsSynthesizeInput(
    ...
    val outputPath: String? = null             // 可选输出文件路径（null 为自动生成）
)
```
...
data class CVModelConfig(
    ...
    val char_dict_path: String? = null,        // 字符字典路径
    val qnn_model_folder_path: String? = null, // QNN 模型文件夹（NPU）
    val qnn_lib_folder_path: String? = null    // QNN 库路径（NPU）
)
```
