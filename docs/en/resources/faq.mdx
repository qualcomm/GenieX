...
<AccordionGroup>
  ...
  <Accordion title="Which runtime is the default?">
    If you don't pass a compute unit, both `llama_cpp` and `qairt` default to `npu`. For llama_cpp's faster HTP + CPU per-tensor scheduler path, pass `hybrid` explicitly.
  </Accordion>
