...
Control which hardware device processes your model via a friendly alias.

```kotlin
val device_id: String?  // "cpu" | "gpu" | "npu" | "hybrid" | null
```

**Available Values** (also exposed as [`DeviceIdValue`](../../../bindings/android/app/src/main/java/com/geniex/sdk/bean/InputPluginBase.kt)):
...
| `"npu"`    | Pinned Hexagon session (`HTP0`). Deterministic layout; slower than `hybrid` on LLMs.              |
| `"hybrid"` | `llama_cpp` per-tensor HTP+CPU scheduler. JNI forces `nGpuLayers = 999`. Fastest LLM path.        |

<Note>
QAIRT (`plugin_id = "qairt"`) only exposes its NPU device. If you pass
`"cpu"`, `"gpu"`, or `"hybrid"` with a QAIRT model the JNI logs a
warning and falls back to NPU — it will not fail the call.
</Note>
