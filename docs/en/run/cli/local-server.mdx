...
By default, GenieX resets mutable model state for every chat-completion request.
`GenieX-KeepCache: true` retains raw state for compatibility, but it does not
validate that a later request belongs to the same conversation. Use the managed
protocol when a client can edit, prune, branch, retry or cancel its history.
...
`exact_extension`, `branch`, `session_switch`, or `parent_mismatch`. If a
generation fails, is cancelled, or disconnects before that final record, its
provisional state is cleared and must not be reused.
...
Version one supports scalar text messages with `system`, `user`, and
