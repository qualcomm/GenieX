...
int32_t LlamaLlm::create_impl(const geniex_LlmCreateInput* input) {
    ...
    // Speculative decoding (MTP): optional, keyed on a non-empty draft model path.
    // Failure to set it up is non-fatal — we log and fall back to plain decoding.
...
// Load the draft (MTP head) model and wire an MTP draft context that shares the
// target KV cache. Mirrors the manual setup in llama.cpp's server-context.cpp
// for the `--spec-type draft-mtp` + separate-drafter-GGUF path.
int32_t LlamaLlm::setup_speculative(const geniex_ModelConfig& config, Device device, const char* device_id) {
    ...
    // Pin the drafter to the same device(s) as the target; without this it falls
    // back to the default placement (which can grab HTP and break the MTP graph).
    ...
    // MTP draft context: same context tuning as the target, plus the MTP wiring
    // (shared KV via ctx_other, no rollback snapshots).
    ...
    if (!this->draft_ctx) {
        GENIEX_LOG_ERROR("failed to create MTP draft context");
        return GENIEX_ERROR_COMMON_MODEL_LOAD;
    }

    this->spec_n_max = config.spec_n_draft > 0 ? config.spec_n_draft : 3;

    common_params_speculative spar;
    spar.types         = {COMMON_SPECULATIVE_TYPE_DRAFT_MTP};
    spar.draft.n_max   = this->spec_n_max;
    ...
    GENIEX_LOG_INFO(
        "speculative decoding (MTP) enabled: draft_model={}, n_draft={}", config.spec_draft_model, this->spec_n_max);
    return GENIEX_SUCCESS;
}
