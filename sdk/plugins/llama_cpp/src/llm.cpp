...
static bool is_intel_vulkan_device(ggml_backend_dev_t device) {
  ...
  bool has_intel = (name.find("Intel") != std::string::npos ||
                    desc.find("Intel") != std::string::npos);
  bool has_vulkan = (name.find("Vulkan") != std::string::npos ||
...
// Helper function to get model block count from GGUF metadata without loading
// full model
...
LlamaLlm::~LlamaLlm() {
  ...
  // Free threadpools
  if (threadpool)
    this->threadpool_free_fn(threadpool);
  if (threadpool_batch)
    this->threadpool_free_fn(threadpool_batch);
}
...
int32_t LlamaLlm::create_impl(const ml_LlmCreateInput *input) {
  ...
  std::transform(model_path_lower.begin(), model_path_lower.end(),
                 model_path_lower.begin(), ::tolower);
  bool is_gpt_oss_model = (model_path_lower.find("gpt") != std::string::npos) &&
                          (model_path_lower.find("oss") != std::string::npos);
  ...
  if (is_gpt_oss_model) {
    ...
    // and .bias)
    this->tensor_overrides[0] = {"\\.ffn_(up|down|gate)_exps\\.(weight|bias)",
                                 ggml_backend_cpu_buffer_type()};
    this->tensor_overrides[1] = {nullptr, nullptr}; // Null terminator
    mpar.tensor_buft_overrides = this->tensor_overrides;
    GENIEX_LOG_INFO("GPT OSS model detected - MoE expert tensors "
  ...
  if (input->device_id) {
    ...
    if (!devices.empty()) {
      ...
      GENIEX_LOG_INFO("Using {} device(s): {}", devices.size(),
                      input->device_id);
    } else {
    ...
    for (size_t i = 0; i < ggml_backend_dev_count(); ++i) {
      ...
      if (!dev)
        continue; // Skip null/failed devices (e.g., HTP init failure)
  ...
  cpar.n_ubatch =
      config.n_ubatch > 0 ? config.n_ubatch : default_config.n_ubatch;
  cpar.n_seq_max =
      config.n_seq_max > 0 ? config.n_seq_max : default_config.n_seq_max;
  cpar.n_threads =
      config.n_threads > 0 ? config.n_threads : default_config.n_threads;
  cpar.n_threads_batch = config.n_threads_batch > 0
                             ? config.n_threads_batch
                             : default_config.n_threads_batch;
  ...
  struct ggml_threadpool_params tpp_batch =
      ggml_threadpool_params_default(cpar.n_threads_batch);
  struct ggml_threadpool_params tpp =
      ggml_threadpool_params_default(cpar.n_threads);
  ...
  if (cpar.n_threads_batch != cpar.n_threads) {
    ...
    if (!this->threadpool_batch) {
      GENIEX_LOG_ERROR("Batch threadpool create failed: n_threads {}",
                       tpp_batch.n_threads);
      return ML_ERROR_COMMON_MODEL_LOAD;
    }
...
int32_t LlamaLlm::save_kv_cache(const ml_KvCacheSaveInput *input,
                                ml_KvCacheSaveOutput *_) {
  return llama_state_save_file(this->ctx, input->path, nullptr, 0)
             ? ML_SUCCESS
             : ML_ERROR_COMMON_UNKNOWN;
}

int32_t LlamaLlm::load_kv_cache(const ml_KvCacheLoadInput *input,
                                ml_KvCacheLoadOutput *_) {
  size_t out;
...
int32_t LlamaLlm::apply_chat_template(const ml_LlmApplyChatTemplateInput *input,
  ...
  std::string template_override =
      this->chat_template_str ? *this->chat_template_str : "";
  common_chat_templates_ptr tmpls =
      common_chat_templates_init(this->model, template_override, "", "");
  ...
  if (input->tools && strlen(input->tools) > 0) {
    inputs.tools = common_chat_tools_parse_oaicompat(
        nlohmann::ordered_json::parse(std::string(input->tools)));
  }
...
int32_t LlamaLlm::generate(const ml_LlmGenerateInput *input,
  ...
  bool has_input_ids =
      input->input_ids != nullptr && input->input_ids_count > 0;
  ...
  ml_GenerationConfig cfg =
      input->config ? *input->config : ml_GenerationConfig{};
  ...
  if (has_input_ids) {
    ...
    prompt_ids.assign(input->input_ids,
                      input->input_ids + input->input_ids_count);
  } else {
    // Use text tokenization path
    try {
      prompt_ids =
          common_tokenize(vocab, std::string(input->prompt_utf8), true, true);
      {
  ...
  GENIEX_LOG_DEBUG("prefix match: past_prompt_tokens size: {}, prompt_len: {}, "
                   "match_len: {}",
                   past_prompt_tokens.size(), prompt_len, match_len);
  ...
  if (match_len < (int)this->past_prompt_tokens.size()) {
    if (match_len < this->n_past_global - this->n_past) {
      ...
      GENIEX_LOG_INFO("prefix match: n_past_global rollback to: {}",
                      this->n_past_global);
    } else {
  ...
  auto process = [&](std::vector<llama_token> &embd) {
    ...
    if (this->n_past + (int)embd.size() >= n_ctx) {
      ...
      llama_memory_seq_add(mem, 0, n_keep + n_discard, this->n_past,
                           -n_discard);
      this->n_past -= n_discard;

      GENIEX_LOG_INFO("Context shifting - discarding {} tokens, n_keep: {}, "
                      "this->n_past before: {}, this->n_past after: "
                      "{}",
                      n_discard, n_keep, n_past_before, this->n_past);
    }
  ...
  for (size_t i = 0; i < embd_inp.size(); ++i) {
    common_sampler_accept(this->sampler, embd_inp[i],
  ...
  while ((int)generated_tokens.size() < cfg.max_tokens) {
    ...
    int n = llama_token_to_piece(vocab, id, token_buf, sizeof(token_buf) - 1, 0,
                                 this->allow_special_tokens);
    if (n < 0)
      return ML_ERROR_LLM_GENERATION_FAILED;
    token_buf[n] = '\0';
    ...
    for (int i = 0; i < cfg.stop_count; ++i) {
      if (cfg.stop[i] && strcmp(token_buf, cfg.stop[i]) == 0) {
        ...
        profiler.set_stop_reason(
            common::StopReason::ML_STOP_REASON_STOP_SEQUENCE);
        stop_matched = true;
        break;
      }
    }
  ...
  this->past_prompt_tokens.insert(this->past_prompt_tokens.end(),
                                  embd_inp.begin(), embd_inp.end());
  this->past_prompt_tokens.insert(this->past_prompt_tokens.end(),
...
void LlamaLlm::set_sampler(const ml_SamplerConfig *cfg) {
  ...
  if (cfg) {
    ...
    s.penalty_repeat =
        (cfg->repetition_penalty != 0.0f) ? cfg->repetition_penalty : 1.0f;
    s.penalty_present =
        (cfg->presence_penalty != 0.0f) ? cfg->presence_penalty : 0.0f;
    s.penalty_freq =
        (cfg->frequency_penalty != 0.0f) ? cfg->frequency_penalty : 0.0f;
